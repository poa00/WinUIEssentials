...
static HWND GetHwnd(winrt::Microsoft::UI::Xaml::Window window)
{
    HWND hwnd{};
...
static winrt::Microsoft::UI::Windowing::AppWindow GetAppWindow(HWND hwnd)
{
...
namespace winrt::WinUI3Example::implementation
{
    ...
    {
        m_appWindow.Resize({ 
            std::clamp(value, MinWidth(), MaxWidth()), 
            Height() 
        });
    }
    int WindowEx::Height()
    ...
    void WindowEx::Height(int value)
    {
        m_appWindow.Resize({
            Width(),
            std::clamp(value, MinHeight(), MaxHeight())
        });
    }
    ...
    LRESULT WindowEx::onGetMinMaxInfo(WPARAM, LPARAM pMinMaxInfo)
    {
        ...
        auto const dpi = Dpi();
        AdjustWindowRectExForDpi(&rect, GetWindowLongPtr(m_hwnd, GWL_STYLE), false, GetWindowLongPtr(m_hwnd, GWL_EXSTYLE), dpi);
        if (m_minWidth)
    ...
    winrt::Windows::Graphics::RectInt32 WindowEx::getRect(winrt::Windows::Foundation::Rect const& bound, double scale)
    {
    ...
    void WindowEx::updateDragRegion()
    {
        ...
        for (auto [element, _] : s_sizeChangeHandlers)
        {
            ...
            rectArray.push_back(getRect(rect, scale));
        }
